...
defmodule AshAi.LiveLLM.StreamingTest do
  ...
  defmodule StreamItem do
    use Ash.Resource, domain: AshAi.LiveLLM.StreamingTest.TestDomain, data_layer: Ash.DataLayer.Ets
  ...
  describe "OpenAI streaming" do
    ...
    test "streams content chunks" do
      ...
      content_events = Enum.filter(events, fn
        {:content, _} -> true
        _ -> false
      end)
      ...
      done_events = Enum.filter(events, fn
        {:done, _} -> true
        _ -> false
      end)
    ...
    test "streams tool call and result events" do
      ...
      tool_call_events = Enum.filter(events, fn
        {:tool_call, _} -> true
        _ -> false
      end)

      tool_result_events = Enum.filter(events, fn
        {:tool_result, _} -> true
        _ -> false
      end)
    ...
    test "streams iteration events for multi-turn" do
      ...
      iteration_events = Enum.filter(events, fn
        {:iteration, _} -> true
        _ -> false
      end)
