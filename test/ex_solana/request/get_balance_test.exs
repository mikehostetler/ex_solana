defmodule ExSolana.RPC.GetBalanceTest do
  ...
  describe "get_balance/2" do
    test "builds correct request with valid public key and default options" do
      ...
      assert Request.get_balance(binary_key) == expected_request
      assert Request.get_balance(encoded_key) == expected_request
    end
    ...
    test "builds correct request with valid public key and custom options" do
      ...
      assert Request.get_balance(encoded_key, opts) == expected_request
    end

    test "returns error with invalid base58 encoded key" do
      invalid_key = "invalid_base58_key"

      assert {:error, error_message} = Request.get_balance(invalid_key)
    ...
    test "returns error with short binary key" do
      ...
      assert {:error, error_message} = Request.get_balance(short_key)
      assert error_message =~ "Invalid key"
    end
    ...
    test "returns error with long binary key" do
      ...
      assert {:error, error_message} = Request.get_balance(long_key)
      assert error_message =~ "Invalid key"
    end
    ...
    test "returns error with invalid options" do
      ...
      assert {:error, error} = Request.get_balance(valid_key, invalid_opts)

      assert error =~ "invalid value for :commitment option"
    end

    test "solana validator can accept the get_balance request", %{
      client: client,
      payer: payer
    } do
      public_key = pubkey!(payer)

      request = Request.get_balance(public_key, commitment: "confirmed")

      assert {:ok, balance} = RPC.send(client, request)
