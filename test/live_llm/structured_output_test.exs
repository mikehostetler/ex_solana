...
defmodule AshAi.LiveLLM.StructuredOutputTest do
    ...
    actions do
      action :analyze_sentiment, Sentiment do
        ...
        run AshAi.Actions.prompt("openai:gpt-4o-mini",
              prompt: {"You are a sentiment analyzer. Respond only with the sentiment.", "Analyze the sentiment of: <%= @input.arguments.text %>"}
            )
      end
      ...
      action :extract_number, :integer do
        ...
        run AshAi.Actions.prompt("openai:gpt-4o-mini",
              prompt: {"You extract numbers from text.", "Extract the main number from: <%= @input.arguments.text %>"}
            )
      end
      ...
      action :extract_entities, {:array, ExtractedEntity} do
        ...
        run AshAi.Actions.prompt("openai:gpt-4o-mini",
              prompt: {"You are a named entity extractor. Extract people, organizations, and locations.", "Extract entities from: <%= @input.arguments.text %>"}
            )
      end
      ...
      action :classify_topic, :atom do
        ...
        run AshAi.Actions.prompt("openai:gpt-4o-mini",
              prompt: {"You classify text into topics.", "Classify this text: <%= @input.arguments.text %>"}
            )
      end
        ...
        run AshAi.Actions.prompt("openai:gpt-4o-mini",
              prompt: fn input, _ctx ->
                ReqLLM.Context.new([
                  ...
                  ReqLLM.Context.user("Summarize in #{input.arguments.max_words} words or fewer: #{input.arguments.text}")
                ])
              end
            )
      end
  ...
  describe "OpenAI structured outputs" do
    ...
    test "enum return type - sentiment analysis" do
      ...
      result =
        AnalysisResource
        |> Ash.ActionInput.for_action(:analyze_sentiment, %{text: "I love this product! It's amazing!"})
    ...
    test "integer return type - number extraction" do
      ...
      result =
        AnalysisResource
        |> Ash.ActionInput.for_action(:extract_number, %{text: "There are 42 apples in the basket"})
        |> Ash.run_action!(domain: TestDomain)
  ...
  describe "Anthropic structured outputs" do
    ...
    test "enum return type - sentiment analysis" do
      ...
      result =
        AnalysisResource
        |> Ash.ActionInput.for_action(:analyze_sentiment_anthropic, %{text: "This is terrible, I hate it!"})
        |> Ash.run_action!(domain: TestDomain)
  ...
  describe "error handling" do
    ...
    test "handles negative sentiment correctly" do
      ...
      result =
        AnalysisResource
        |> Ash.ActionInput.for_action(:analyze_sentiment, %{text: "This is the worst experience ever. Completely disappointed."})
        |> Ash.run_action!(domain: TestDomain)
    ...
    test "handles neutral sentiment correctly" do
      ...
      result =
        AnalysisResource
        |> Ash.ActionInput.for_action(:analyze_sentiment, %{text: "The meeting is scheduled for 3pm tomorrow."})
        |> Ash.run_action!(domain: TestDomain)
