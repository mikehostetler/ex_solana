defmodule ExSolana.SPL.Governance do
  @moduledoc """
  ...
  """
  def id, do: ExSolana.pubkey!("GovER5Lthms3bLBqWub97yVrMmEogzX7xNjdXpPPCVZw")

  @doc """
  The governance program's test instance ID. This can be used to set up test DAOs.
  """
  def test_id, do: ExSolana.pubkey!("GTesTBiEWE32WHXXE2S4XbZvA5CrEc4xs6ZgRe895dP")
  ...
  def validate_account(other) do
    {:error, "expected a ExSolana.Account, got #{inspect(other)}"}
  end
  ...
  def validate_instruction(other) do
    {:error, "expected a ExSolana.Instruction, got: #{inspect(other)}"}
  end
  ...
  @create_account_governance_schema [
    ...
    config: [
      type: {:custom, ExSolana.Helpers, :validate, [@governance_config_schema]},
      required: true,
      doc: """
  ...
  govern an upgradable ExSolana program.
  ...
  defp bpf_loader, do: ExSolana.pubkey!("BPFLoaderUpgradeab1e11111111111111111111111")
  ...
  calling `ExSolana.SPL.Governance.remove_transaction/1`.
  ...
  # TODO replace with with ExSolana.clock() once `solana` package is updated
  defp clock, do: ExSolana.pubkey!("SysvarC1ock11111111111111111111111111111111")
