defmodule Hako.Adapter.ETS do
  @moduledoc """
  Hako Adapter using ETS (Erlang Term Storage) for in-memory storage.

  ## Direct usage

      iex> filesystem = Hako.Adapter.ETS.configure(name: ETSFileSystem)
  ...
  ## Direct usage with eternal tables
      ...
      iex> filesystem = Hako.Adapter.ETS.configure(name: ETSFileSystem, eternal: true)
      iex> start_supervised(filesystem)
      iex> :ok = Hako.write(filesystem, "test.txt", "Hello World")
      iex> {:ok, "Hello World"} = Hako.read(filesystem, "test.txt")
    ...
    defimpl Collectable do
      def into(%{config: config, path: path} = stream) do
        collector_fn = fn
          acc, {:cont, elem} ->
            GenServer.call(
              Hako.Registry.via(Hako.Adapter.ETS, config.name),
  ...
  defp do_start_link(config) do
    GenServer.start_link(__MODULE__, config, name: Hako.Registry.via(__MODULE__, config.name))
  end
  ...
  defp call_server(config, message) do
    GenServer.call(Hako.Registry.via(__MODULE__, config.name), message)
  end
      ...
      case check_path_in_directory(file_path, normalized_path) do
        {:direct_child, name} when name not in [".", ".."] ->
          if Map.has_key?(acc, name) do
            if match?(%Hako.Stat.Dir{}, acc[name]) do
              acc
            else
              case content do
                map when is_map(map) ->
