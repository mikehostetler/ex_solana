defmodule Hako.Adapter.InMemory.Versioning do
  @moduledoc """
  Versioning wrapper for InMemory adapter.

  This module provides a unified versioning interface for the InMemory adapter,
  translating the new Hako.Adapter.Versioning behaviour calls to the 
  existing InMemory adapter versioning functions.
  """

  @behaviour Hako.Adapter.Versioning

  alias Hako.Adapter.InMemory

  @impl Hako.Adapter.Versioning
  def commit(_config, _message \\ nil, _opts \\ []) do
    # InMemory doesn't have a traditional "commit" concept like Git
    # Since InMemory writes are immediate, "commit" is effectively a no-op
  ...
  def revisions(config, path \\ ".", opts \\ []) do
    case InMemory.list_versions(config, path) do
      {:ok, versions} ->
        ...
        filtered_versions =
          ...
          |> apply_limit(limit)
          |> Enum.map(&in_memory_version_to_versioning_format/1)
  ...
  def read_revision(config, path, revision, _opts \\ []) do
    InMemory.read_version(config, path, revision)
  end
  ...
  def rollback(config, revision, opts \\ []) do
    ...
    if path do
      ...
      case InMemory.read_version(config, path, revision) do
        {:ok, content} ->
          InMemory.write(config, path, content, [])
