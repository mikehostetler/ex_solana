defmodule Hako.Adapter.Local do
  @moduledoc """
  ...
  ## Direct usage
      ...
      iex> filesystem = Hako.Adapter.Local.configure(prefix: prefix)
  ...
  ### Examples
      ...
      {:ok, %File.Stream{line_or_bytes: 1_024, ...}} = Hako.read_stream(filesystem, "test.txt", chunk_size: 1_024)

      # with custom file read modes
      {:ok, %File.Stream{mode: [{:encoding, :utf8}, :binary], ...}} = Hako.read_stream(filesystem, "test.txt", modes: [encoding: :utf8])

  """
  import Bitwise
  alias Hako.Visibility.UnixVisibilityConverter
  alias Hako.Visibility.PortableUnixVisibilityConverter, as: DefaultVisibilityConverter
  alias Hako.Errors
      ...
      {:ok, files} ->
        contents =
          for file <- files,
            ...
            struct =
              case stat.type do
                :directory -> Hako.Stat.Dir
                :regular -> Hako.Stat.File
              end
  ...
  defp clear_dir_or_file(config, %Hako.Stat.Dir{name: dir}),
    do: delete_directory(config, dir, recursive: true)

  defp clear_dir_or_file(config, %Hako.Stat.File{name: name}),
    do: delete(config, name)

  defp full_path(config, path) do
    Hako.RelativePath.join_prefix(config.prefix, path)
  end
