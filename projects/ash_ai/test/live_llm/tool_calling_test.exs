...
defmodule AshAi.LiveLLM.ToolCallingTest do
  ...
  defmodule Item do
    use Ash.Resource, domain: AshAi.LiveLLM.ToolCallingTest.TestDomain, data_layer: Ash.DataLayer.Ets
  ...
  describe "OpenAI tool calling" do
    ...
    test "single tool call - list items" do
      ...
      messages = [
        Context.system("You are a helpful inventory assistant. Use the available tools to answer questions."),
      ...
      assert result.final_text =~ "Widget" or result.final_text =~ "gadget" or result.final_text =~ "item"
    end
    ...
    test "create item tool call" do
      ...
      messages = [
        Context.system("You are a helpful inventory assistant. Use the create_item tool to create items. After creating, respond with what you created."),
    ...
    test "handles tool with no results gracefully" do
      ...
      assert result.iterations >= 1
      assert result.final_text =~ ~r/no items|empty|nothing/i or length(result.tool_calls_made) >= 1
    end
  end
