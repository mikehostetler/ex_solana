defmodule Hako.Adapter.ETSTest do
  use ExUnit.Case, async: true
  import Hako.AdapterTest
  # doctest Hako.Adapter.ETS

  setup do
    filesystem = Hako.Adapter.ETS.configure(name: :ets_test)
  ...
  describe "write" do
    test "success", %{filesystem: filesystem} do
      ...
      :ok = Hako.Adapter.ETS.write(config, "test.txt", "Hello World", [])

      assert {:ok, "Hello World"} = Hako.Adapter.ETS.read(config, "test.txt")
    end

    test "folders are automatically created if missing", %{filesystem: filesystem} do
      {_, config} = filesystem

      :ok = Hako.Adapter.ETS.write(config, "folder/test.txt", "Hello World", [])

      assert {:ok, "Hello World"} = Hako.Adapter.ETS.read(config, "folder/test.txt")
    end

    test "visibility", %{filesystem: filesystem} do
      {_, config} = filesystem

      :ok = Hako.Adapter.ETS.write(config, "public.txt", "Hello World", visibility: :public)
      :ok = Hako.Adapter.ETS.write(config, "private.txt", "Hello World", visibility: :private)

      assert {:ok, :public} = Hako.Adapter.ETS.visibility(config, "public.txt")
      assert {:ok, :private} = Hako.Adapter.ETS.visibility(config, "private.txt")
    end
  end
  ...
  describe "read" do
    ...
    test "file not found", %{filesystem: filesystem} do
      ...
      assert {:error, %Hako.Errors.FileNotFound{file_path: "nonexistent.txt"}} =
               Hako.Adapter.ETS.read(config, "nonexistent.txt")
    end
  end
  ...
  describe "delete" do
    test "success", %{filesystem: filesystem} do
      ...
      :ok = Hako.Adapter.ETS.write(config, "test.txt", "Hello World", [])
      assert :ok = Hako.Adapter.ETS.delete(config, "test.txt")

      assert {:error, %Hako.Errors.FileNotFound{file_path: "test.txt"}} =
               Hako.Adapter.ETS.read(config, "test.txt")
    end

    test "successful even if no file to delete", %{filesystem: filesystem} do
      {_, config} = filesystem

      assert :ok = Hako.Adapter.ETS.delete(config, "nonexistent.txt")
    end
  end
  ...
  describe "move" do
    test "success", %{filesystem: filesystem} do
      ...
      :ok = Hako.Adapter.ETS.write(config, "source.txt", "Hello World", [])
      assert :ok = Hako.Adapter.ETS.move(config, "source.txt", "destination.txt", [])

      assert {:error, %Hako.Errors.FileNotFound{file_path: "source.txt"}} =
               Hako.Adapter.ETS.read(config, "source.txt")

      assert {:ok, "Hello World"} = Hako.Adapter.ETS.read(config, "destination.txt")
    end
  end
  ...
  describe "copy" do
    test "success", %{filesystem: filesystem} do
      ...
      :ok = Hako.Adapter.ETS.write(config, "source.txt", "Hello World", [])
      assert :ok = Hako.Adapter.ETS.copy(config, "source.txt", "destination.txt", [])
      assert {:ok, "Hello World"} = Hako.Adapter.ETS.read(config, "source.txt")
      assert {:ok, "Hello World"} = Hako.Adapter.ETS.read(config, "destination.txt")
    end
  end
  ...
  describe "file_exists" do
    test "existing file", %{filesystem: filesystem} do
      ...
      :ok = Hako.Adapter.ETS.write(config, "test.txt", "Hello World", [])
      assert {:ok, :exists} = Hako.Adapter.ETS.file_exists(config, "test.txt")
    end

    test "non-existing file", %{filesystem: filesystem} do
      {_, config} = filesystem

      assert {:ok, :missing} = Hako.Adapter.ETS.file_exists(config, "nonexistent.txt")
    end
  end
  ...
  describe "list_contents" do
    test "success", %{filesystem: filesystem} do
      ...
      :ok = Hako.Adapter.ETS.create_directory(config, "dir1", [])

      {:ok, contents} = Hako.Adapter.ETS.list_contents(config, ".")
  ...
  describe "create_directory" do
    test "success", %{filesystem: filesystem} do
      ...
      assert :ok = Hako.Adapter.ETS.create_directory(config, "new_dir", [])
      assert {:ok, :exists} = Hako.Adapter.ETS.file_exists(config, "new_dir")
    end
  end
  ...
  describe "delete_directory" do
    test "success", %{filesystem: filesystem} do
      ...
      :ok = Hako.Adapter.ETS.create_directory(config, "dir_to_delete", [])
      assert :ok = Hako.Adapter.ETS.delete_directory(config, "dir_to_delete", [])
      assert {:ok, :missing} = Hako.Adapter.ETS.file_exists(config, "dir_to_delete")
    end

    test "recursive delete", %{filesystem: filesystem} do
      {_, config} = filesystem

      :ok = Hako.Adapter.ETS.create_directory(config, "parent_dir", [])
      :ok = Hako.Adapter.ETS.write(config, "parent_dir/file.txt", "Content", [])

      assert :ok = Hako.Adapter.ETS.delete_directory(config, "parent_dir", recursive: true)
      assert {:ok, :missing} = Hako.Adapter.ETS.file_exists(config, "parent_dir")
      assert {:ok, :missing} = Hako.Adapter.ETS.file_exists(config, "parent_dir/file.txt")
    end
  end
  ...
  describe "eternal functionality" do
    ...
    test "eternal tables persist data independently of adapter process" do
      ...
      # Write via adapter
      :ok = Hako.Adapter.ETS.write(config, "adapter.txt", "adapter data", [])
      assert {:ok, "adapter data"} = Hako.Adapter.ETS.read(config, "adapter.txt")
    ...
    test "non-eternal tables do not survive process termination" do
      ...
      {:ok, _new_pid} = Hako.Adapter.ETS.start_link(regular_filesystem)

      # Data should be gone (regular ETS table died with process)
      assert {:error, %Hako.Errors.FileNotFound{file_path: "temporary.txt"}} =
               Hako.Adapter.ETS.read(config, "temporary.txt")
    end

    test "eternal configuration defaults to false" do
      filesystem = Hako.Adapter.ETS.configure(name: :default_test)
      {_, config} = filesystem
    ...
    test "eternal configuration can be explicitly set to true" do
      filesystem = Hako.Adapter.ETS.configure(name: :explicit_eternal, eternal: true)
      {_, config} = filesystem
    ...
    test "eternal configuration can be explicitly set to false" do
      filesystem = Hako.Adapter.ETS.configure(name: :explicit_regular, eternal: false)
      {_, config} = filesystem
