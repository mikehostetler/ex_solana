defmodule Hako.Adapter.GitTest do
  ...
  describe "filesystem operations in manual mode" do
    ...
    test "write and read files", %{filesystem: fs} do
      assert :ok = Hako.write(fs, "test.txt", "Hello World")
      assert {:ok, "Hello World"} = Hako.read(fs, "test.txt")
    end

    test "delete files", %{filesystem: fs} do
      Hako.write(fs, "test.txt", "content")
      assert :ok = Hako.delete(fs, "test.txt")
      assert {:error, _} = Hako.read(fs, "test.txt")
    end

    test "move files", %{filesystem: fs} do
      Hako.write(fs, "source.txt", "content")
      assert :ok = Hako.move(fs, "source.txt", "dest.txt", [])
      assert {:error, _} = Hako.read(fs, "source.txt")
      assert {:ok, "content"} = Hako.read(fs, "dest.txt")
    end

    test "copy files", %{filesystem: fs} do
      Hako.write(fs, "source.txt", "content")
      assert :ok = Hako.copy(fs, "source.txt", "dest.txt", [])
      assert {:ok, "content"} = Hako.read(fs, "source.txt")
      assert {:ok, "content"} = Hako.read(fs, "dest.txt")
    end

    test "create and delete directories", %{filesystem: fs} do
      assert :ok = Hako.create_directory(fs, "subdir/", [])

      # Git creates .gitkeep files for empty directories
      assert {:ok, ""} = Hako.read(fs, "subdir/.gitkeep")

      assert :ok = Hako.delete_directory(fs, "subdir/", recursive: true)
      assert {:error, _} = Hako.read(fs, "subdir/.gitkeep")
    end
  ...
  describe "versioning operations" do
    ...
    test "commit creates commits with messages", %{filesystem: fs, test_repo: repo_path} do
      Hako.write(fs, "test.txt", "content")
      assert :ok = Hako.commit(fs, "Add test file")
    ...
    test "revisions with limit", %{filesystem: fs} do
      ...
      assert {:ok, revisions} = Hako.revisions(fs, ".", limit: 1)
    ...
    test "read_revision reads historical content", %{filesystem: fs} do
      ...
      {:ok, [revision | _]} = Hako.revisions(fs, "test.txt")

      Hako.write(fs, "test.txt", "version 2")
      Hako.commit(fs, "Second version")

      assert {:ok, "version 1"} = Hako.read_revision(fs, "test.txt", revision.sha)
    ...
    test "rollback resets to previous state", %{filesystem: fs} do
      ...
      {:ok, [target_revision | _]} = Hako.revisions(fs, ".")

      Hako.write(fs, "test.txt", "version 2")
      Hako.commit(fs, "Second version")

      assert :ok = Hako.rollback(fs, target_revision.sha)
      assert {:ok, "version 1"} = Hako.read(fs, "test.txt")
    end
    ...
    test "rollback specific file", %{filesystem: fs} do
      ...
      assert :ok = Hako.rollback(fs, target_revision.sha, path: "file1.txt")
      # rolled back
  ...
  describe "unsupported operations on non-Git adapters" do
    test "commit returns unsupported for Local adapter" do
      local_fs = Hako.Adapter.Local.configure(prefix: @tmp_dir)
      assert {:error, :unsupported} = Hako.commit(local_fs)
    end

    test "revisions returns unsupported for Local adapter" do
      local_fs = Hako.Adapter.Local.configure(prefix: @tmp_dir)
      assert {:error, :unsupported} = Hako.revisions(local_fs)
    end

    test "read_revision returns unsupported for Local adapter" do
      local_fs = Hako.Adapter.Local.configure(prefix: @tmp_dir)
      assert {:error, :unsupported} = Hako.read_revision(local_fs, "file.txt", "abc123")
    end

    test "rollback returns unsupported for Local adapter" do
      local_fs = Hako.Adapter.Local.configure(prefix: @tmp_dir)
      assert {:error, :unsupported} = Hako.rollback(local_fs, "abc123")
    end
  end
  ...
  describe "error handling" do
    ...
    test "git operations handle command failures gracefully", %{test_repo: repo_path} do
      ...
      assert {:error, _message} = Hako.read_revision(fs, "nonexistent.txt", "invalid_sha")
    end
  end
