defmodule ExSolana.SPL.Token do
  @moduledoc """
  Functions for interacting with ExSolana's [Token
  ...
  def id, do: ExSolana.pubkey!("TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA")
  ...
  @doc """
  Translates the result of a `ExSolana.RPC.Request.get_account_info/2` into a
  ...
  @transfer_schema [
    ...
    multi_signers: [
      type: {:list, {:custom, ExSolana.Key, :check, []}},
      doc: "signing accounts if the `owner` is a `ExSolana.SPL.Token.MultiSig` account"
    ],
    amount: [
      type: :pos_integer,
      required: true,
  ...
  @set_authority_schema [
    ...
    multi_signers: [
      type: {:list, {:custom, ExSolana.Key, :check, []}},
      doc: "signing accounts if the `authority` is a `ExSolana.SPL.Token.MultiSig` account"
    ]
  ]
  ...
  #   ExSolana.SPL.Token.DecodeInstruction.decode_instruction(data)
  # end

  # @impl ExSolana.ProgramBehaviour
  def analyze_ix(decoded_parsed_ix, confirmed_transaction) do
    ExSolana.Spl.Token.AnalyzeInvocation.analyze_ix(decoded_parsed_ix, confirmed_transaction)
  end
end
