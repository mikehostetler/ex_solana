defmodule ExSolana.Native.SystemProgram.NonceTest do
  ...
  describe "withdraw/1" do
    test "can withdraw lamports from a nonce account", %{
      ...
      tx = %Transaction{
        instructions: [
          SystemProgram.create_account(
            lamports: ExSolana.lamports_per_sol(),
            space: space,
            program_id: SystemProgram.id(),
            from: pubkey!(payer),
            new: pubkey!(new)
          ...
          SystemProgram.Nonce.withdraw(
            ...
            authority: pubkey!(payer),
            to: pubkey!(payer),
            lamports: div(ExSolana.lamports_per_sol(), 2)
          )
        ],
        signers: [payer, new],
        blockhash: blockhash,
      ...
      assert info["lamports"] == div(ExSolana.lamports_per_sol(), 2)
    end
  end
end
