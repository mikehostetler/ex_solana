defmodule ExSolana.RPC.Middleware do
  @moduledoc false
  ...
  defp handle_response({:ok, %{status: status} = response}, request) when status in @success do
  ...
  defp decode_result({"getTransaction", %{"transaction" => tx} = _result}) when is_map(tx) do
    # tx =
    #   tx
