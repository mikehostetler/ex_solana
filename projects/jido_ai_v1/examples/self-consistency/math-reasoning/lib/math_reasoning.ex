defmodule Examples.SelfConsistency.MathReasoning do
  @moduledoc """
  Basic Self-Consistency example demonstrating mathematical reasoning using the SelfConsistency runner.
  ...
  1. Generating k diverse reasoning paths (with higher temperature)
  2. Extracting and normalizing answers from each path
  3. Analyzing path quality (coherence, completeness, confidence)
  4. Voting across paths to select the most reliable answer
  ...
  - Automated answer extraction and normalization
  ...
  @doc """
  Run the complete example with the classic "bat and ball" problem using the SelfConsistency runner.
  """
  def run do
    IO.puts("\n" <> String.duplicate("=", 70))
    IO.puts("  Self-Consistency: Mathematical Reasoning (Runner-Based)")
    ...
    IO.puts("🔧 **Method:** SelfConsistency runner with automated voting\n")
    IO.puts(String.duplicate("-", 70) <> "\n")
  ...
  @doc """
  Solve a problem using Self-Consistency with the SelfConsistency runner.
  ...
  - `opts` - Options:
    ...
    - `:voting_strategy` - :majority | :confidence_weighted | :quality_weighted | :hybrid (default: :majority)
  ...
  def solve(problem, opts \\ []) do
    ...
    IO.puts("🚀 **Running Self-Consistency with runner...**\n")

    # Use custom reasoning function to generate domain-specific paths
    reasoning_fn = fn _i ->
      paths = generate_reasoning_paths(problem, sample_count, temperature)
      {:ok, path_list} = paths
  ...
  defp display_result(result) do
    IO.puts(String.duplicate("=", 70))
    IO.puts("\n✅ **Self-Consistency Complete (via Runner)**\n")
    ...
    # Determine winning paths (those with the winning answer)
    winning_answer = result.answer

    result.paths
    |> Enum.with_index(1)
    |> Enum.each(fn {path, idx} ->
      winner_mark = if path.answer == winning_answer, do: " ✓", else: ""
