defmodule Hako.Adapter.S3 do
  ...
  def write(%Config{} = config, path, contents, opts) do
    path = clean_path(Hako.RelativePath.join_prefix(config.prefix, path))
  ...
  def copy(%Config{} = source_config, source, %Config{} = dest_config, destination, opts) do
    source = clean_path(Hako.RelativePath.join_prefix(source_config.prefix, source))
    destination = clean_path(Hako.RelativePath.join_prefix(dest_config.prefix, destination))
  ...
  def list_contents(%Config{} = config, path) do
    base_path = clean_path(Hako.RelativePath.join_prefix(config.prefix, path))
