defmodule Depot.Adapter.InMemory.VersioningTest do
  use ExUnit.Case, async: true

  alias Depot.Adapter.InMemory
  alias Depot.Adapter.InMemory.Versioning

  setup do
    name = :"test_memory_versioning_#{:rand.uniform(10000)}"
    {_adapter, config} = InMemory.configure(name: name)

    start_supervised!({InMemory, config})

    {:ok, config: config}
  end

  describe "commit/3" do
    test "always returns :ok for InMemory adapter", %{config: config} do
  ...
  describe "revisions/3" do
    ...
    test "lists versions with standardized format", %{config: config} do
      ...
      {:ok, _v1} = InMemory.write_version(config, "test.txt", "version 1", [])
      {:ok, _v2} = InMemory.write_version(config, "test.txt", "version 2", [])
      ...
      assert revision.author_email == "inmemory@depot.local"
    ...
    test "supports limit option", %{config: config} do
      ...
      for i <- 1..5 do
        {:ok, _} = InMemory.write_version(config, "test.txt", "version #{i}", [])
      end
    ...
    test "supports time range filtering", %{config: config} do
      # Create a version
      {:ok, _} = InMemory.write_version(config, "test.txt", "version 1", [])
  ...
  describe "read_revision/4" do
    test "reads content from specific version", %{config: config} do
      {:ok, version_id} = InMemory.write_version(config, "test.txt", "test content", [])
  ...
  describe "rollback/3" do
    test "rollbacks single file to previous version", %{config: config} do
      ...
      {:ok, version_id} = InMemory.write_version(config, "test.txt", "original content", [])
      InMemory.write(config, "test.txt", "modified content", [])

      # Verify file was modified
      assert {:ok, "modified content"} = InMemory.read(config, "test.txt")

      # Rollback to original version
      assert :ok = Versioning.rollback(config, version_id, path: "test.txt")

      # Verify rollback worked
      assert {:ok, "original content"} = InMemory.read(config, "test.txt")
    end
