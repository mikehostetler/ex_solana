...
# ========================================
# Install Coding Agents (if not already installed)
# ========================================
if ! command -v amp &> /dev/null; then
    echo "Installing Amp coding agent..."
    curl -fsSL https://ampcode.com/install.sh | bash 2>/dev/null || echo "Amp install failed (non-critical)"
...
if ! command -v claude &> /dev/null; then
    ...
    curl -fsSL https://claude.ai/install.sh | bash 2>/dev/null || echo "Claude Code install failed (non-critical)"
    # Add to PATH for this session
    export PATH="$HOME/.local/bin:$PATH"
fi
...
if [ -n "$GIT_REPO" ]; then
    ...
    if [ ! -d "$REPO_DIR" ]; then
        ...
        if git clone "$GIT_REPO" "$REPO_DIR"; then
