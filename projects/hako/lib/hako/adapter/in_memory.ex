defmodule Hako.Adapter.InMemory do
  @moduledoc """
  Hako Adapter using an `Agent` for in memory storage.

  ## Direct usage

      iex> filesystem = Hako.Adapter.InMemory.configure(name: InMemoryFileSystem)
      iex> start_supervised(filesystem)
      iex> :ok = Hako.write(filesystem, "test.txt", "Hello World")
      iex> {:ok, "Hello World"} = Hako.read(filesystem, "test.txt")
  ...
  defmodule AgentStream do
    ...
    defimpl Enumerable do
      def reduce(%{config: config, path: path, chunk_size: chunk_size} = stream, a, b) do
        case Hako.Adapter.InMemory.read(config, path) do
          {:ok, contents} ->
            contents
            |> Hako.chunk(chunk_size)
      ...
      def into(%{config: config, path: path} = stream) do
        ...
        fun = fn
          ...
          list, :done ->
            ...
            Hako.Adapter.InMemory.write(config, path, contents, [])
            stream
  ...
  def start_link(%Config{} = config) do
    Agent.start_link(fn -> {%{}, %{}, %{}} end, name: Hako.Registry.via(__MODULE__, config.name))
  end
  ...
  def write(config, path, contents, opts) do
    ...
    Agent.update(Hako.Registry.via(__MODULE__, config.name), fn state ->
  ...
  def read(config, path) do
    Agent.get(Hako.Registry.via(__MODULE__, config.name), fn state ->
  ...
  def move(%Config{} = config, source, destination, opts) do
    ...
    Agent.get_and_update(Hako.Registry.via(__MODULE__, config.name), fn state ->
    ...
    contents =
      Agent.get(Hako.Registry.via(__MODULE__, config.name), fn state ->
        ...
        for {path, {content, meta}} <- paths do
          struct =
            case content do
              %{} -> %Hako.Stat.Dir{size: 0}
              bin when is_binary(bin) -> %Hako.Stat.File{size: byte_size(bin)}
            end
  ...
  def clear(%Config{} = config) do
    Agent.update(Hako.Registry.via(__MODULE__, config.name), fn _ -> {%{}, %{}, %{}} end)
  end
  ...
  def write_version(%Config{} = config, path, contents, opts) do
    ...
    Agent.get_and_update(Hako.Registry.via(__MODULE__, config.name), fn {files, dirs, versions} ->
  ...
  def read_version(%Config{} = config, path, version_id) do
    Agent.get(Hako.Registry.via(__MODULE__, config.name), fn {_files, _dirs, versions} ->
  ...
  def delete_version(%Config{} = config, path, version_id) do
    Agent.update(Hako.Registry.via(__MODULE__, config.name), fn {files, dirs, versions} ->
