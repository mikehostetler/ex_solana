defmodule ExSolana.TestValidator do
  @moduledoc """
  A [ExSolana Test Validator](https://docs.solana.com/developing/test-validator)
  ...
  Since `ExSolana.TestValidator` uses the `solana-test-validator` binary, you'll
  need to have the [ExSolana tool
  suite](https://docs.solana.com/cli/install-solana-cli-tools) installed.

  ## How to Use

  You can use the `ExSolana.TestValidator` directly or in a supervision tree.
  ...
  See `ExSolana.TestValidator.start_link/1` for more details.
  """
  use GenServer
  ...
  @schema [
    ...
    # dynamic_port_range: [type: :string, default: "1024-65534"],
  ...
  @doc """
  Starts a `ExSolana.TestValidator` process linked to the current process.
  ...
  @doc """
  Stops a `ExSolana.TestValidator` process.
  ...
  @doc """
  Gets the state of a `ExSolana.TestValidator` process.
  ...
  def init(opts) do
    with ex_path when not is_nil(ex_path) <- System.find_executable("solana-test-validator"),
      ...
      if ExSolana.Config.get(:verbose) do
        Logger.debug("Solana test validator command: #{command_string}")
      end

      Port.monitor(port)

      Logger.info("Waiting for Solana test validator to start...")

      case wait_for_validator_ready(port) do
        :ok ->
          Logger.info("Solana test validator is ready")
          {:ok, %{port: port, latest_output: nil, exit_status: nil, ledger: ledger}}

        {:error, reason} ->
          Logger.error("Failed to start Solana test validator: #{reason}")
          {:stop, reason}
      end
    else
      false ->
  ...
  defp wait_for_validator_ready(port, timeout \\ 30_000) do
    start_time = System.monotonic_time(:millisecond)
    wait_for_validator_ready_loop(port, start_time, timeout)
  end

  defp wait_for_validator_ready_loop(port, start_time, timeout) do
    receive do
      {^port, {:data, output}} ->
        if String.contains?(output, "Processed Slot") do
          :ok
        else
          wait_for_validator_ready_loop(port, start_time, timeout)
        end
